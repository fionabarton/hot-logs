...
public class BattleItems : MonoBehaviour {
	...
	public void AddFunctionToButton(Action<int, Item> functionToPass, string messageToDisplay, Item item) {
		...
		_.playerActions.playerButtonCS[0].onClick.AddListener(delegate { functionToPass(0, item); });
		_.playerActions.playerButtonCS[1].onClick.AddListener(delegate { functionToPass(1, item); });
		_.playerActions.playerButtonCS[2].onClick.AddListener(delegate { functionToPass(2, item); });
